...
public class HttpImp {
    ...
    public static void token(final Context context, String url, String auth_code) {
        ...
        String sign = DuDaiParamHelper.duDaiCreateSign("UTF-8", paramObj, DuDaiParamHelper.CP_SECRET);
...
//                }
    ...
    public static void duDaiGameUrl(final Context context, String url, final JYDCallback callback) {
        SortedMap paramObj = new TreeMap<>();
        paramObj.put("game_id", LoadConfig.JY_GAMEID + "");
        paramObj.put("channel_id", LoadConfig.JY_CHANNEL_ID + "");
        paramObj.put("cp_id", LoadConfig.JY_CP_ID + "");
        ...
        HttpRequestUtil.okGetFormRequest(url, paramObj, new HttpRequestUtil.DataCallBack() {
            @Override
            public void requestSuccess(String result) throws Exception {
                JYDuDaiGameUrlBean bean = GsonUtils.GsonToBean(result, JYDuDaiGameUrlBean.class);
    ...
    public static void duDaiOrderMethod(final Context context, String url, final JYDPayParam payParam, final JYDCallback callback) {
        ProgressBarUtil.showProgressBar((Activity) context);
        SortedMap Param = DuDaiParamHelper.duDaiMapParam(context);
        ...
        Param.put("is_test", 2);
        String sign = DuDaiParamHelper.duDaiCreateSign("UTF-8", Param, DuDaiParamHelper.GAME_SECRET);
        Param.put("sign", sign);
        ...
        HttpRequestUtil.okPostJsonRequest(url, Param, new HttpRequestUtil.DataCallBack() {
            ...
            public void requestSuccess(String result) throws Exception {
                ...
                JYDuDaiOrderBean bean = GsonUtils.GsonToBean(result, JYDuDaiOrderBean.class);
                if (bean.getStatus().equals("success")) {
                    callback.callback(JYDStatusCode.SUCCESS, bean);
                } else {
                    ToastUtil.showLongHideSoftInput(context, bean.getMessage());
     ...
     * 独代上报
     */
    public static void duDaiLog(final Context context, String url, String type) {
        SortedMap Param = DuDaiParamHelper.duDaiMapParam(context);
        Param.put("type", type);
        String sign = DuDaiParamHelper.duDaiCreateSign("UTF-8", Param, DuDaiParamHelper.GAME_SECRET);
        Param.put("sign", sign);
